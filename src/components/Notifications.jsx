      ...
      <div className="requestsTap">
        ...
        <>
          <button onClick={() => setRequest("RegisterRequests")}>
            Register Requests
            { registerRequests.length > 0 && <span className="requestCount">{registerRequests.length}</span>}
          </button>

          <button onClick={() => setRequest("LeaveRequests")}>
              Leave Requests
            {leaveRequests.length > 0 && <span className="requestCount">{leaveRequests.length}</span>}
          </button>

          <button onClick={() => setRequest("TransferRequests")}>
            Transfer Requests
            {transferRequests.length > 0 && <span className="requestCount">{transferRequests.length}</span>}
          </button>
        </>}

          <button onClick={() => setRequest("AppliedLeaveForms")}>
          Applied Leave Forms
            {appliedLeaveForms.length > 0 && <span className="requestCount">{appliedLeaveForms.length}</span>}
          </button>

          <button onClick={() => setRequest("AppliedTransferForms")}>
          Applied Transfer Forms
            {appliedTransferForms.length > 0 && <span className="requestCount">{appliedTransferForms.length}</span>}
          </button>

      </div>

      <div className="requests">
        {(request === "" || request === "LeaveRequests") && <LeaveRequests allLeaveFormRequests={leaveRequests} setAllLeaveFormRequests={setLeaveRequests}/>}
        {request === "RegisterRequests" && <RegisterRequests requests={registerRequests} setRequests={setRegisterRequests}/>}
        {request === "TransferRequests" && <TransferRequests />}
        {request === "AppliedLeaveForms" && <AppliedLeaveForms appliedLeaveForms={appliedLeaveForms} setAppliedLeaveForms={setAppliedLeaveForms} />}
